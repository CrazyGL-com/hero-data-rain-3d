...
function approach(cur, target, dt, response = 0.18) {
...
function buildAtlas(THREE, chars, fontFamily) {
...
export default function RainStage(props) {
    ...
    React.useEffect(() => {
        ...
        return () => {
            ...
            disposeField(state);
            if (state.atlasTex?.dispose)
                state.atlasTex.dispose();
    ...
    React.useEffect(() => {
        ...
        const tick = (now) => {
            ...
            // Background clear.
            u.u_time.value = state.shaderTime;
            state.renderer.setClearColor(new THREE.Color(bg[0], bg[1], bg[2]), p.transparent ? 0 : 1);
            state.renderer.render(state.scene, state.camera);
        };
        state.lastT = performance.now();
        state.rafHandle = requestAnimationFrame(tick);
    ...
    return (_jsx("canvas", { ref: canvasRef, className: "crazygl-datarain-canvas", "aria-hidden": "true" }));
}
/* ─── Field build + dispose ──────────────────────────────────────────── */
function disposeField(state) {
    if (state.mesh && state.scene)
        state.scene.remove(state.mesh);
    if (state.geometry?.dispose)
        state.geometry.dispose();
    if (state.material?.dispose)
        state.material.dispose();
    state.mesh = null;
    state.geometry = null;
    state.material = null;
}
function buildField(state, p) {
    const THREE = state.THREE;
